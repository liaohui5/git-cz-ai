# `model_name` → `model_name` 字段重命名设计规格

> 日期：2026-08-20 · 状态：已批准

## 需求

将 `AiArgs` struct 的 `model_name` 字段改名为 `model_name`，并同步更新 CLI 参数名 `--model-name` → `--model-name` 及全部文档引用。

## 设计决策（经用户确认）

| 决策点 | 选择 | 理由 |
|--------|------|------|
| CLI 参数名 | 字段与 CLI 参数都改：`--model-name` → `--model-name` | 用户明确选择；属破坏性变更，需同步文档 |
...
- 第 38 行：`model_name: String,` → `model_name: String,`（`#[arg(long)]` 属性保持）
- 第 62 行：`"model": args.model_name,` → `"model": args.model_name,`

### 文档（6 个文件，`--model-name` → `--model-name`，`model_name` → `model_name`）

| 文件 | 改动处 |
|------|--------|
| `README.md` | 用法示例、参数表、env 示例（4 处 CLI 参数） |
| `AGENTS.md` | §5.5 数据流图、§11 附录（2 处 CLI 参数） |
| `docs/superpowers/specs/2026-08-20-ai-subcommand-design.md` | `model_name` 字段、`--model-name` 引用 |
| `docs/superpowers/plans/2026-08-20-ai-subcommand.md` | 字段、使用点、参数引用 |
| `docs/superpowers/specs/2026-08-20-waiting-for-response-design.md` | `args.model_name` 代码块 |
| `docs/superpowers/plans/2026-08-20-waiting-for-response.md` | `args.model_name`、命令示例 |

## 行为规格

| 项目 | 改动前 | 改动后 |
|------|--------|--------|
| CLI 参数 | `--model-name <MODEL>` | `--model-name <MODEL>` |
| Rust 字段 | `args.model_name` | `args.model_name` |
